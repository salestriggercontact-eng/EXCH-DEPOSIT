...
function signToken(user) {
  return jwt.sign({ id: user._id, type: 'user' }, process.env.JWT_SECRET, { expiresIn: '7d' });
}
...
exports.getProfile = async (req, res) => {
  try {
    ...
    // Include all fields including customFields
    return res.json({
      success: true,
      user: {
        id: user._id,
        ...
        upiId: user.upiId || '',
        customFields: user.customFields || {}, // <-- critical
      },
    });
  } catch (err) {
    console.error(err);
